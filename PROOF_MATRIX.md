...
| Historical product UI | Earlier mandate/probation/promotion/revocation interface | Git history before the product-only revision | Superseded; not present in the current website |
| Product-only workflow | User-supplied endpoint → saved policy → six live decisions → inspectable report; no public fixtures, winner, authority or forced revocation | Public report `cnr_5ab58552`, clean browser DOM/language check, persistence reload, single-agent regression test | Verified publicly, 2026-08-26 |
